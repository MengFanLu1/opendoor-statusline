...
impl AutoConfigurator {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let config_dir = home.join(".claude/opendoor-statusline");
        Ok(Self { config_dir })
    }
    ...
    pub fn setup_opendoor-statusline(
        ...
        // Add opendoor-statusline segments if not present
        use crate::config::{
        ...
        let has_usage = config
            .segments
            .iter()
            .any(|s| matches!(s.id, SegmentId::OpenDoorUsage));
        let has_sub = config
            .segments
            .iter()
            .any(|s| matches!(s.id, SegmentId::OpenDoorSubscription));
